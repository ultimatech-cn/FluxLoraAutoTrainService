...
logger = setup_logger('AIToolkitOneShotTrainer')
...
class AIToolkitOneShotTrainer(BaseTrainer):
    ...
    def train(self, task_data: dict) -> bool:
        ...
        self.job_manager.update_job_status(self.config['job_id'], 'ONESHOT_TRAIN', JobStatus.Processing.value)
        # 1. create dataset
        ...
        try:
            ...
            self.job_manager.update_job_status(self.config['job_id'], 'ONESHOT_TRAIN', JobStatus.Done.value)
            return True
        except Exception as e:
            job.cleanup()
            logger.error(f"Training failed: {str(e)}")
            self.job_manager.update_job_status(self.config['job_id'], 'ONESHOT_TRAIN', JobStatus.Failed.value)
